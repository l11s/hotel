    ...
    if (settingsStore.tabbar.enable) {
        tabbarStore.add(val).then(() => {
            if (proxy.$refs[`tab-${activedTabId.value}`]) {
                scrollTo(proxy.$refs[`tab-${activedTabId.value}`][0].offsetLeft)
                tabbarScrollTip()
            }
        })
    }
...
function tabbarScrollTip() {
    if (proxy.$refs['tab-container'].$el.clientWidth > proxy.$refs['tabs'].clientWidth && !storage.local.has('tabbarScrollTip')) {
...
<template>
    <div class="tabbar-container">
        ...
        >
            <transition-group ref="tab-container" :name="!isDragging ? 'tabbar' : null" tag="div" class="tab-container" :class="{'dragging': isDragging}">
                <div
                    v-for="element in tabbarList" :key="element.tabId"
                    :ref="`tab-${element.tabId}`" class="tab" :class="{
                ...
                >
                    ...
                    <div class="tab-content">
                        <div :key="settingsStore.titleFirst && element.tabId == activedTabId ? element.title : generateI18nTitle(element.i18n, element.title)" class="title">{{ settingsStore.titleFirst && element.tabId == activedTabId ? element.title : generateI18nTitle(element.i18n, element.title) }}</div>
